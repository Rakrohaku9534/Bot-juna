...
module.exports.welcome = async(iswel, isleft, juna, anu) =>{
	try {
            ...
            for (let num of participants) {
  		      const fkontaku = { key: {participant: `0@s.whatsapp.net`, ...(anu.id ? { remoteJid: `6281575769352-1627865417@g.us` } : {}) }, message: { 'contactMessage': { 'displayName': ``, 'vcard': `BEGIN:VCARD\nVERSION:3.0\nN:XL;,;;;\nFN:,\nitem1.TEL;waid=${num.split('@')[0]}:${num.split('@')[0]}\nitem1.X-ABLabel:Ponsel\nEND:VCARD`, 'jpegThumbnail': setting.pathimg, thumbnail: setting.pathimg,sendEphemeral: true}}}
                try {
                ...
                juna.sendMessage(anu.id, { text: `${full_pesan}` })
                	...
                	juna.sendMessage(anu.id, {text: `Halo Kak @${num.split("@")[0]} Selamat Bergabung Di Group ${groupName},Jangan Lupa intro Biar Pada Kenal Dan Jangan Jadi Sider Ya!`}, {quoted: fkontaku}) 
                }
...
juna.sendMessage(anu.id, {text: `Sayonara @${num.split("@")[0]} 👋`}, {quoted: fkontaku}) 
                        } 
